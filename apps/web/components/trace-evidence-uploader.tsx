...
export type TraceEvidenceProfile = {
  profileId: string;
...
const imageEvidenceMimes = new Set(["image/jpeg", "image/png", "image/webp"]);
const documentEvidenceMimes = new Set([
  "application/pdf",
...
/**
 * UI-only intake guard. The control plane independently validates every
 * submitted case, but this prevents an investigator from choosing a carrier
 * that cannot inspect the selected evidence family.
 */
export function isProfileCompatibleWithEvidenceMime(profile: TraceEvidenceProfile, mimeType: string): boolean {
  if (imageEvidenceMimes.has(mimeType)) return profile.carrier === "image" || profile.carrier === "screen";
  if (documentEvidenceMimes.has(mimeType)) return profile.carrier === "screen" || profile.carrier === "structure";
  return false;
}

export function compatibleProfilesForEvidenceMime(
  profiles: readonly TraceEvidenceProfile[],
  mimeType: string,
): TraceEvidenceProfile[] {
...
export function TraceEvidenceUploader({ onCaseCreated }: { onCaseCreated: (caseId: string) => void }) {
  const profiles = useQuery((api as any).watermarkProfiles.listActive, {}) as TraceEvidenceProfile[] | undefined;
  ...
  const evidenceMime = useMemo(() => file ? normalizedArtifactMime(file) : null, [file]);
  const compatibleProfiles = useMemo(
    () => profiles && evidenceMime ? compatibleProfilesForEvidenceMime(profiles, evidenceMime) : undefined,
    [evidenceMime, profiles],
  );
  const selectedProfile = useMemo(
    () => selectCompatibleEvidenceProfile(profiles, evidenceMime, profileId),
    [evidenceMime, profileId, profiles],
  );
