...
Every immutable source version also queues one CPU `content_index` job. This job has no profile secret: it verifies the source SHA-256, records deterministic image/PDF page fingerprints, uploads canonical page previews plus a compact manifest, and completes through the specialized lease-bound endpoint. Office sources currently complete as explicitly unindexed until a pinned Office index renderer is introduced. Keep the normal worker daemon running during a demo; otherwise both the source index and protected-copy download row will remain queued.
...
The deterministic fixture administrator display name is `tongtong`. This is a display fixture, not an authentication bypass: WorkOS still must authenticate a matching subject before any live protected-copy download, upload, or trace action is available.
