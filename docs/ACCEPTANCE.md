...
- A trace case now selects an immutable profile and atomically queues a bounded `trace` job. Its leased worker input contains only the evidence URL plus at most 100 ready, same-profile anonymous issuance candidates (opaque trace handle, server-mapped code, and derived hash); it contains no recipient identity. A candidate is recorded only after detector evidence matches one of those server-resolved bindings.
